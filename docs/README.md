# Spell Checker - Check Yo Self 

🎯 **Live Demo**: [https://vaporjawn.github.io/Spell-Checker/](https://vaporjawn.github.io/Spell-Checker/)

A beautiful, modern spell checker with an intelligent AI-powered correction algorithm.

## ✨ Features

- ⚡ **Lightning Fast**: Instant spell checking with advanced algorithms
- 🎯 **Accurate**: High precision corrections based on context
- 🔒 **Private**: All processing happens in your browser
- 📱 **Responsive**: Works perfectly on all devices
- 🎨 **Beautiful UI**: Modern gradient design with smooth animations
- ⌨️ **Keyboard Shortcuts**: Press Ctrl/Cmd + Enter to check spelling

## 🚀 Usage

1. Visit the [live demo](https://vaporjawn.github.io/Spell-Checker/)
2. Type or paste your text into the textarea
3. Click "Check Spelling" or press Ctrl/Cmd + Enter
4. See corrections applied instantly!

## 🛠️ Technology

- Pure JavaScript (no dependencies)
- CSS3 with custom properties
- Responsive design
- Client-side processing (works offline)

## 📄 License

MIT License - Victor Williams

## 🔗 Links

- [GitHub Repository](https://github.com/Vaporjawn/Spell-Checker)
- [Author GitHub](https://github.com/Vaporjawn)

Built with ❤️ by Victor Williams
